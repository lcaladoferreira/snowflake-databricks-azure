...
def ingest_raw_to_bronze(spark: SparkSession, table_name: str, batch_id: str):
    ...
    checkpoint_path = f"{Config.ADLS_BASE_PATH}/checkpoints/bronze/{table_name}" if Config.EXECUTION_MODE != "demo" else None

    # Read from landing (using Auto Loader in Production/Databricks, standard Parquet for demo/local)
    if Config.EXECUTION_MODE != "demo" and spark.conf.get("spark.databricks.service.client.enabled", "false") == "true":
